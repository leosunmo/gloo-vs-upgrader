...
func writeV1Gloo(sourceFile string, v1VirtualService glooV1.VirtualService) error {
	var err error
	var fileName string
	...
	f, fErr := os.OpenFile(fileName, os.O_CREATE|os.O_WRONLY, 0664)
	if fErr != nil {
		return err
	}
	defer f.Close()
...
func main() {
	...
	if len(flag.Args()) > 0 {
		...
		fmt.Printf("Usage: %s [-k] <one or more YAML files>\n", os.Args[0])
		flag.PrintDefaults()
		os.Exit(1)
	}
	// Read Gloo VS yaml
